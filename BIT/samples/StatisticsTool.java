...
public class StatisticsTool {
	...
	public static void main(String argv[]) {
		...
		} else if (argv[0].equals("-project")) {
			
			System.out.println("in project else");
			
			if (argv.length != 4) {
				System.out.println("in argv check, number of arguments is incorrect for -project flag");
			}
			
			try {
				log_writer = new PrintWriter(new BufferedWriter(new FileWriter(argv[3], true))); //writer for logging
				System.out.println("logWriter in main: " + log_writer); //log_writer is not null here
			} catch (IOException e) {
				System.out.println("exception with log-writer instantiation");
			}

			System.out.println("main started, log file: " + argv[3]);
			
			try {
				File in_dir = new File(argv[1]);
				File out_dir = new File(argv[2]);
				File log_file = new File(argv[3]);
				if (in_dir.isDirectory() && out_dir.isDirectory()) {
					doDynamic(in_dir, out_dir); // currently using dynamic routine for testing purposes
				} else {
					System.out.println("in project directory check");
				}
			} catch (NullPointerException e) {
				System.out.println("in null pointer exception because of input/output/log directory");
			}
		}
