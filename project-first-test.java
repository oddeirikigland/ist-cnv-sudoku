...
public class StatisticsTool {
	...
	public static void doDynamic(File in_dir, File out_dir) {
		...
		for (int i = 0; i < filelist.length; i++) {
			...
			if (filename.endsWith(".class")) {
				...
				ci.addAfter("StatisticsTool", "printToFile", log_writer);
	...
	public static synchronized void printToFile(PrintWriter logWriter) {
		System.out.println("logWriter: " + logWriter);
		logWriter.println("logwritertest");
	}
	...
	public static void main(String argv[]) {
		...
		if (argv[0].equals("-static")) {
			...
			if (argv.length != 4) {
				System.out.println("in argv check");
				printUsage();
			}
			
			try {
				log_writer = new PrintWriter(new BufferedWriter(new FileWriter(argv[3], true)));
				System.out.println("logWriter in main: " + log_writer);
			} catch (IOException e) {
			...
			try {
				...
				System.out.println("in null pointer e");
				printUsage();
			}
		}
